# Generates all of the commands needed to make a Slab Door open.
...
sound_string = f"execute positioned {center_coords} run playsound minecraft:block.piston.contract block @p ~ ~ ~ 2"
...
with open(file, 'w') as file:
    for i in range(6):
        ...
        if i == 0:
            coords_1 = f"{bottom_x} {top_y} {bottom_z} {top_x} {top_y - 1} {top_z}"
            coords_2 = f"{bottom_x} {bottom_y} {bottom_z} {top_x} {bottom_y} {top_z}"
        if i == 2:
            coords_1 = f"{bottom_x} {top_y} {bottom_z} {top_x} {top_y} {top_z}"
            coords_2 = f"{bottom_x} {bottom_y + 1} {bottom_z} {top_x} {bottom_y + 1} {top_z}"
        if i == 4:
        ...
        file.write(f"fill {coords_2} ")
        if i % 2 == 0:
            file.write("red_sandstone_slab[type=top]\n")
        else:
            file.write("air\n")
